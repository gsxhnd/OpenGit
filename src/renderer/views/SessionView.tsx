...
import { useSessionPerConnectionState } from '../hooks/useSessionPerConnectionState'
import { useRemoteShellLifecycle } from '../hooks/useRemoteShellLifecycle'
import { useSessionSftpOperations } from '../hooks/useSessionSftpOperations'
...
export function SessionView() {
  ...
  const { startRemoteShell, disconnect } = useRemoteShellLifecycle(cid, meta, getState, rerender)
  ...
  const {
    openFile, navigateTo, handleNewFolder, submitNewFolder,
    handleRename, handleDelete, handleProperties, handleUpload, handleDownload,
  ...
  useEffect(() => {
    return () => { stateMapRef.current.delete(cid) }
  }, [cid, stateMapRef])
    ...
    <div className={styles.root}>
      ...
      <div className={styles.body}>
        <SftpPane
          ...
          onNewFolder={() => handleNewFolder(setNewFolderValue, setNewFolderPrompt)}
        ...
        <div className={styles.mainCol}>
          ...
          {st.editor && ed && (
            <SessionEditorPanel
              ...
              ed={ed}
              cid={cid}
              onClose={() => { st.editor = null; rerender() }}
      ...
      {newFolderPrompt ? (
        <NewFolderDialog
          ...
          onSubmit={() => { void submitNewFolder(newFolderValue, setNewFolderPrompt) }}
        />
      ) : null}
